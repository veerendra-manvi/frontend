...
const QuizEngine = ({ quizId, questions = [], onComplete }) => {
  ...
  if (isFinished) {
    ...
    return (
      <Card className="text-center py-12 px-8 bg-gradient-to-br from-[#12151e] to-transparent border-brand-primary/20 animate-in zoom-in-95 duration-500">
         ...
         <p className="text-slate-500 mb-10 font-medium">Topic Mastery: {percentage}%</p>

         <div className="grid grid-cols-2 gap-4 mb-10">
            <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
               <span className="text-[10px] font-black uppercase text-slate-600 tracking-widest block mb-2">Accuracy</span>
               <h4 className="text-3xl font-black text-emerald-500">{correctCount}<span className="text-slate-500 text-lg">/{questions.length}</span></h4>
            </div>
            <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
               <span className="text-[10px] font-black uppercase text-slate-600 tracking-widest block mb-2">XP Earned</span>
               <h4 className="text-3xl font-black text-brand-primary flex items-center justify-center gap-1">
                  <Zap size={20} fill="currentColor" /> {xpEarned}
               </h4>
            </div>
         </div>

         <div className="space-y-4">
            <PrimaryButton onClick={handleFullRetry} className="w-full py-5 font-black uppercase tracking-widest text-xs">
               <RotateCcw size={16} /> Full Retake
            </PrimaryButton>
            {correctCount < questions.length && (
              <SecondaryButton onClick={handleRetryIncorrect} className="w-full py-5 border-brand-secondary/30 text-brand-secondary font-black uppercase tracking-widest text-xs">
                 <AlertCircle size={16} /> Retry Incorrect Only
              </SecondaryButton>
            )}
         </div>
      </Card>
    ...
    <Card className="p-0 border-white/5 bg-[#0b0d12] overflow-hidden shadow-2xl">
      ...
      <div className="bg-white/5 px-8 py-5 border-b border-white/5 flex justify-between items-center">
         <div className="flex items-center gap-4">
            ...
            <div>
               <h4 className="text-xs font-black text-white uppercase tracking-[0.2em]">Quiz Protocol</h4>
               <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">Question {currentIndex + 1} of {activeQuestionSet.length}</p>
            </div>
         </div>
         {isRetryingIncorrect && <Badge variant="secondary" className="animate-pulse">Correction Mode</Badge>}
      </div>
                    ...
                    return (
                      ...
                      >
                         ...
                         <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${isSelected ? 'bg-current border-current' : 'border-white/10 group-hover:border-white/20'}`}>
                            {showFeedback && isCorrect && <CheckCircle2 size={14} className="text-[#0b0d12]" />}
                            {showFeedback && isSelected && !isCorrect && <XCircle size={14} className="text-[#0b0d12]" />}
                         </div>
                      </button>
                    );
                  })}
         ...
         <div className="mt-12 flex flex-col md:flex-row gap-6 pt-10 border-t border-white/5 relative">
            <AnimatePresence>
               {showFeedback ? (
                 ...
                 >
                    <div className="flex gap-4 p-5 bg-white/3 rounded-2xl border border-white/5">
                       <div className="p-2 bg-brand-primary/10 rounded-lg h-fit text-brand-primary"><AlertCircle size={18} /></div>
                       <div>
                          <p className="text-[10px] font-black uppercase text-brand-primary tracking-widest mb-1">Knowledge Sync</p>
                          <p className="text-sm text-slate-400 font-medium leading-relaxed">{currentQuestion.explanation}</p>
                       </div>
                    </div>
                    <PrimaryButton onClick={handleNext} className="w-full md:w-auto px-12 py-4 shadow-2xl">
                       {currentIndex === activeQuestionSet.length - 1 ? 'See Results' : 'Next Question'}
                    </PrimaryButton>
                 </motion.div>
               ) : (
